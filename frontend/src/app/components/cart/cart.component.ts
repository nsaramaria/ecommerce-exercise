...
@Component({
  selector: 'app-cart',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, ProductImageComponent],
  template: `
                ...
                <article class="item">
                  <div class="thumb" [style.background]="item.product.cardBgColor">
                    <app-product-image
                      ...
                      [fallbackColor]="item.product.tubeColor"
                      [alt]="item.product.name"
                      [height]="78"
                      [scale]="0.55"></app-product-image>
                  </div>
                  <div class="info">
  ...
  styles: [`
    ...
    .title { font-size: clamp(40px, 6vw, 60px); line-height: 1; margin: 0; }
    ...
    .name { font-family: var(--font-serif); font-size: 18px; margin: 0 0 2px; }
    .desc {
    ...
    .line-total { font-size: 16px; font-weight: 500; min-width: 60px; text-align: right; }
    ...
    .summary-title { font-family: var(--font-serif); font-size: 22px; margin: 0 0 20px; }
    .row {
      display: flex; justify-content: space-between;
      padding: 6px 0; font-size: 14px; color: var(--text-muted);
    }
    .row.total { font-size: 18px; color: var(--text); font-weight: 500; }
    .est {
...
export class CartComponent {
  ...
  inc(id: number, current: number): void { this.cart.updateQuantity(id, current + 1); }
  dec(id: number, current: number): void { this.cart.updateQuantity(id, current - 1); }
